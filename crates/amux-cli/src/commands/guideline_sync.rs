...
const GITHUB_GUIDELINES_TREE_URL: &str =
    "https://api.github.com/repos/mkurman/tamux/git/trees/main?recursive=1";
const GITHUB_RAW_MAIN_URL: &str = "https://raw.githubusercontent.com/mkurman/tamux/main";
...
pub(super) async fn fetch_remote_guideline_documents() -> Result<Vec<RemoteGuidelineDocument>> {
    ...
    let user_agent = format!("tamux-cli/{}", env!("CARGO_PKG_VERSION"));
    ...
    let response = client
        ...
        .send()
        .await
        .context("failed to query tamux guideline tree")?
        .error_for_status()
        .context("tamux guideline tree request returned an error status")?;

    let tree = response
        .json::<GitTreeResponse>()
        .await
        .context("failed to parse tamux guideline tree response")?;
    if tree.truncated {
        anyhow::bail!("tamux guideline tree response was truncated by GitHub");
    }
    ...
    if documents.is_empty() {
        anyhow::bail!("no markdown guidelines were found in the tamux repository");
    }
    Ok(documents)
}

            ...
            framed
                .send(ClientMessage::GatewayAck {
                    ack: zorai_protocol::GatewayAck {
                        ...
                        accepted: true,
                        detail: Some("zorai-mcp bootstrap acknowledged".to_string()),
                    },
                })
                .await
...
async fn read_next_roundtrip_message<T>(framed: &mut Framed<T, ZoraiCodec>) -> Result<DaemonMessage>
where
    T: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin,
{
    loop {
...
pub(super) async fn connect_daemon(
    ...
    #[cfg(unix)]
    {
        ...
        let stream = tokio::net::UnixStream::connect(&path)
            .await
            .with_context(|| format!("cannot connect to zorai daemon at {}", path.display()))?;
    ...
    #[cfg(windows)]
    {
        ...
        let stream = tokio::net::TcpStream::connect(&addr)
            .await
            .with_context(|| format!("cannot connect to zorai daemon on {addr}"))?;
        Ok(Framed::new(stream, ZoraiCodec))
    }
}
...
mod tests {
    ...
    async fn daemon_roundtrip_framed_skips_gateway_control_messages() {
        ...
        let mut client = Framed::new(client, ZoraiCodec);

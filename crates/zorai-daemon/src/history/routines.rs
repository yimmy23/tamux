...
fn map_routine_definition_row(row: &db::Row) -> anyhow::Result<RoutineDefinitionRow> {
    Ok(RoutineDefinitionRow {
        id: row.get(0)?,
        title: row.get(1)?,
        description: row.get(2)?,
        enabled: row.get::<i64>(3)? != 0,
        paused_at: row.get::<Option<i64>>(4)?.map(|value| value.max(0) as u64),
        schedule_expression: row.get(5)?,
        target_kind: row.get(6)?,
        target_payload_json: row.get(7)?,
        schema_version: row.get::<i64>(8)?.max(1) as u32,
        next_run_at: row.get::<Option<i64>>(9)?.map(|value| value.max(0) as u64),
        last_run_at: row.get::<Option<i64>>(10)?.map(|value| value.max(0) as u64),
        last_result: row.get(11)?,
        last_error: row.get(12)?,
        last_success_summary: row.get(13)?,
        created_at: row.get::<i64>(14)?.max(0) as u64,
        updated_at: row.get::<i64>(15)?.max(0) as u64,
    })
}

fn map_routine_run_row(row: &db::Row) -> anyhow::Result<RoutineRunRow> {
    Ok(RoutineRunRow {
        id: row.get(0)?,
        routine_id: row.get(1)?,
        trigger_kind: row.get(2)?,
        status: row.get(3)?,
        started_at: row.get::<i64>(4)?.max(0) as u64,
        finished_at: row.get::<Option<i64>>(5)?.map(|value| value.max(0) as u64),
...
impl HistoryStore {
    ...
    pub async fn list_routine_definitions(&self) -> Result<Vec<RoutineDefinitionRow>> {
        ...
        Ok(rows
            .iter()
            .filter_map(|row| map_routine_definition_row(row).ok())
            .collect())
    }
    ...
    pub async fn get_routine_definition(&self, id: &str) -> Result<Option<RoutineDefinitionRow>> {
        let row = self
            ...
            .await?;
        row.map(|row| map_routine_definition_row(&row)).transpose()
    }
    ...
    pub async fn list_routine_runs(
        ...
        let rows = self
            ...
            .query(
                ...
                db::db_params![routine_id, limit],
            )
            .await?;
        Ok(rows
            .iter()
            .filter_map(|row| map_routine_run_row(row).ok())
            .collect())
    }
    ...
    pub async fn get_routine_run(&self, run_id: &str) -> Result<Option<RoutineRunRow>> {
        let row = self
            ...
            .await?;
        row.map(|row| map_routine_run_row(&row)).transpose()
    }
}

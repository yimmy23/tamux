...
fn map_goal_run_row_db(row: &db::Row) -> anyhow::Result<GoalRun> {
    ...
    Ok(GoalRun {
        ...
        status: parse_goal_run_status(&row.get::<String>(4)?),
        priority: parse_task_priority(&row.get::<String>(5)?),
        created_at: row.get::<i64>(6)? as u64,
        updated_at: row.get::<i64>(7)? as u64,
        started_at: row.get::<Option<i64>>(8)?.map(|value| value as u64),
        ...
        current_step_index: row.get::<i64>(15)? as usize,
        ...
        replan_count: row.get::<i64>(16)? as u32,
        max_replans: row.get::<i64>(17)? as u32,
        ...
        child_task_ids,
        child_task_count: row.get::<i64>(26)? as u32,
        approval_count: row.get::<i64>(27)? as u32,
        awaiting_approval_id: row.get(28)?,
        policy_fingerprint: row.get(29)?,
        approval_expires_at: row.get::<Option<i64>>(30)?.map(|value| value as u64),
        ...
        dossier: dossier_json
            .as_deref()
            .and_then(|json| serde_json::from_str(json).ok()),
        total_prompt_tokens: row.get::<i64>(37)? as u64,
        total_completion_tokens: row.get::<i64>(38)? as u64,
        estimated_cost_usd: row.get(39)?,
        model_usage: serde_json::from_str(&model_usage_json).unwrap_or_default(),
        autonomy_level: parse_autonomy_level(&row.get::<String>(41)?),
        authorship_tag: row
            .get::<Option<String>>(42)?
            .map(|value| parse_authorship_tag(&value)),
    })
}

fn map_goal_run_thread_ref_row_db(row: &db::Row) -> anyhow::Result<GoalRunThreadRef> {
...
/// Facade-form goal-run upsert generic over any executor (standalone
/// connection or transaction). Keep INSERT column lists / step reaping /
/// event reaping in lockstep with the row mapper.
async fn upsert_goal_run_exec<E: db::DbExecutor + ?Sized>(
    exec: &mut E,
    goal_run: &GoalRun,
) -> anyhow::Result<()> {
    ...
    exec.execute(
        ...
        db::db_params![
            ...
            goal_run.goal.clone(),
            goal_run.client_request_id.clone(),
            ...
            goal_run.plan_summary.clone(),
            goal_run.reflection_summary.clone(),
            memory_updates_json,
            goal_run.generated_skill_path.clone(),
            goal_run.last_error.clone(),
            goal_run.failure_cause.clone(),
            goal_run.stopped_reason.clone(),
            child_task_ids_json,
            goal_run.child_task_count as i64,
            goal_run.approval_count as i64,
            goal_run.awaiting_approval_id.clone(),
            goal_run.policy_fingerprint.clone(),
            goal_run.approval_expires_at.map(|value| value as i64),
            goal_run.containment_scope.clone(),
            goal_run.compensation_status.clone(),
            goal_run.compensation_summary.clone(),
            goal_run.active_task_id.clone(),
    ...
    exec.execute(
        ...
        db::db_params![goal_run.id.clone(), now_ts() as i64],
    )
    .await?;
...
impl HistoryStore {
    pub async fn upsert_goal_run(&self, goal_run: &GoalRun) -> Result<()> {
        ...
        upsert_goal_run_exec(&mut *txn, goal_run).await?;
        txn.commit().await?;
        Ok(())
    }
    ...
    pub(crate) async fn pause_interrupted_goal_runs_on_restart(
        ...
        let mut update_values = vec![db::Value::Text(
            goal_run_status_to_str(GoalRunStatus::Paused).to_string(),
        )];
        update_values.extend(goal_run_ids.iter().cloned().map(db::Value::Text));
    ...
    pub(crate) async fn goal_run_replan_count(&self, goal_run_id: &str) -> Result<Option<u32>> {
        let row = self
            ...
            .await?;
        Ok(match row {
            Some(row) => Some(row.get::<i64>(0)?.max(0) as u32),
            None => None,
        })
    }
    ...
    pub(crate) async fn goal_run_task_context(
        ...
        let row = self
            ...
            .await?;
        row.map(|row| -> anyhow::Result<GoalRunTaskContextRef> {
            Ok(GoalRunTaskContextRef {
                current_step_index: row.get::<i64>(0)?.max(0) as usize,
    ...
    pub(crate) async fn goal_run_progress_metrics(
        ...
        row.map(|row| -> anyhow::Result<GoalRunProgressMetricsRef> {
            Ok(GoalRunProgressMetricsRef {
                steps_total: row.get::<i64>(0)?.max(0) as usize,
                steps_completed: row.get::<i64>(1)?.max(0) as usize,
            })
        })
        .transpose()
    }
    ...
    pub(crate) async fn goal_run_policy_context(
        ...
        let row = self
            ...
            .await?;
        row.map(|row| -> anyhow::Result<GoalRunPolicyContextRef> {
            Ok(GoalRunPolicyContextRef {
                goal: row.get(0)?,
                title: row.get(1)?,
                current_step_title: row.get(2)?,
                steps_total: row.get::<i64>(3)?.max(0) as usize,
                steps_completed: row.get::<i64>(4)?.max(0) as usize,
            })
        })
        .transpose()
    }
    ...
    pub(crate) async fn goal_run_todo_context(
        ...
        let Some(current_step_index) = self
            .read_db
            .query_opt(
            ...
            Some(goal_step_id) => self
                ...
                .query_opt(
                    "SELECT ordinal, id, status
                     ...
                     LIMIT 1",
                    db::db_params![goal_run_id, goal_step_id],
                )
                .await?
                .map(|row| -> anyhow::Result<(usize, String, String)> {
                    Ok((
                ...
                .query_opt(
                    "SELECT ordinal, id, status
                     ...
                     WHERE goal_run_id = ?1
                       ...
                       AND deleted_at IS NULL
                     LIMIT 1",
                    db::db_params![goal_run_id, current_step_index],
                )
                .await?
                .map(|row| -> anyhow::Result<(usize, String, String)> {
                    Ok((
    ...
    pub(crate) async fn goal_run_workspace_runtime_ref(
        ...
        let row = self
            ...
            .await?;
        row.map(|row| -> anyhow::Result<GoalRunWorkspaceRuntimeRef> {
    ...
    pub(crate) async fn goal_run_compaction_scope_ref(
        ...
        let Some(mut scope) = self
            ...
            .await?
            .map(|row| -> anyhow::Result<GoalRunCompactionScopeRef> {
        ...
        let event_rows = self
            ...
            .await?;
        scope.recent_events = event_rows
            .iter()
            .map(|row| row.get::<String>(0))
            .collect::<anyhow::Result<Vec<_>>>()?;
        Ok(Some(scope))
    ...
    pub(crate) async fn list_goal_run_status_refs_for_statuses(
        ...
        rows.iter()
            .map(|row| {
                Ok((
                    row.get::<String>(0)?,
                    parse_goal_run_status(&row.get::<String>(1)?),
                ))
            })
            .collect()
    }
        ...
        rows.iter()
            .map(|row| {
                Ok(GoalRunOperationalRef {
                    ...
                    status: parse_goal_run_status(&row.get::<String>(1)?),
                    title: row.get(2)?,
                    current_step_index: row.get::<i64>(3)?.max(0) as usize,
                    step_count: row.get::<i64>(4)?.max(0) as usize,
                })
            })
            .collect()
    }
        ...
        rows.iter()
            .map(|row| {
                ...
                Ok(GoalRunQuietRecoveryRef {
                    id: row.get(0)?,
                    status: parse_goal_run_status(&row.get::<String>(1)?),
                    created_at: row.get::<i64>(2)?.max(0) as u64,
                    started_at: row.get::<Option<i64>>(3)?.map(|value| value.max(0) as u64),
                    root_thread_id: row.get(5)?,
                    execution_thread_ids: deserialize_goal_run_thread_ids(
                        &thread_id,
                        execution_thread_ids_json,
                    ),
                    current_step_index: row.get::<i64>(7)?.max(0) as usize,
    ...
    pub(crate) async fn list_goal_runs_for_statuses_updated_before(
        ...
        let mut values = vec![db::Value::Integer(updated_at_lte as i64)];
        values.extend(
            statuses
                .iter()
                .map(|status| db::Value::Text(goal_run_status_to_str(*status).to_string())),
    ...
    pub(crate) async fn list_active_goal_runs_for_start_request(
        ...
        values.extend(status_values.into_iter().map(db::Value::Text));
        if let Some(client_request_id) = client_request_id {
            sql.push_str(" AND goal_runs.client_request_id = ?");
            values.push(db::Value::Text(client_request_id));
        } else {
            ...
            let thread_value = thread_id.map(db::Value::Text).unwrap_or(db::Value::Null);
    ...
    pub(crate) async fn concierge_goal_context(&self) -> Result<ConciergeGoalContext> {
        ...
        let latest_goal_run = if let Some(goal_run_id) = latest_id {
            let lean: Option<ConciergeGoalRunLean> = self
                .interactive_read_db
                .query_opt(
                ...
                .await?
                .map(|row| -> anyhow::Result<ConciergeGoalRunLean> {
                    Ok(ConciergeGoalRunLean {
                        ...
                        created_at: row.get::<i64>(5)? as u64,
                        updated_at: row.get::<i64>(6)? as u64,
                        started_at: row.get::<Option<i64>>(7)?.map(|v| v as u64),
                        completed_at: row.get::<Option<i64>>(8)?.map(|v| v as u64),
                        plan_summary: row.get(9)?,
                        reflection_summary: row.get(10)?,
                        current_step_index: row.get::<i64>(11)?.max(0) as usize,
                    })
                })
                .transpose()?;

            if let Some(lean) = lean {
                let current_step_title: Option<String> = self
                    .interactive_read_db
                    .query_opt(
                        "SELECT title FROM goal_run_steps \
                         WHERE goal_run_id = ?1 AND ordinal = ?2 AND deleted_at IS NULL",
                        db::db_params![lean.id.clone(), lean.current_step_index as i64],
                    )
                    .await?
                    .map(|row| row.get::<String>(0))
                    .transpose()?;
    ...
    async fn list_goal_runs_filtered(&self, goal_run_id: Option<String>) -> Result<Vec<GoalRun>> {
        ...
        let mut step_values = Vec::<db::Value>::new();
        if let Some(goal_run_id) = goal_run_id.as_deref() {
            step_sql.push_str(" AND goal_run_id = ?");
            step_values.push(db::Value::Text(goal_run_id.to_string()));
        }
        step_sql.push_str(" ORDER BY goal_run_id ASC, ordinal ASC");

        let step_rows = self
            .read_db
            .query(&step_sql, db::Params::Positional(step_values))
            .await?;
        let mut step_map = std::collections::HashMap::<String, Vec<GoalRunStep>>::new();
        for row in step_rows.iter() {
            let goal_run_id = row.get::<String>(1)?;
            let step = GoalRunStep {
                id: row.get(0)?,
                position: row.get::<i64>(2)? as usize,
                title: row.get(3)?,
                instructions: row.get(4)?,
                kind: parse_goal_run_step_kind(&row.get::<String>(5)?),
                success_criteria: row.get(6)?,
                session_id: row.get(7)?,
                status: parse_goal_run_step_status(&row.get::<String>(8)?),
                task_id: row.get(9)?,
                summary: row.get(10)?,
                error: row.get(11)?,
                started_at: row.get::<Option<i64>>(12)?.map(|value| value as u64),
                completed_at: row.get::<Option<i64>>(13)?.map(|value| value as u64),
            };
        ...
        let mut event_values = Vec::<db::Value>::new();
        if let Some(goal_run_id) = goal_run_id.as_deref() {
            event_sql.push_str(" AND goal_run_id = ?");
            event_values.push(db::Value::Text(goal_run_id.to_string()));
        }
        event_sql.push_str(" ORDER BY timestamp ASC");

        let event_rows = self
            .read_db
            .query(&event_sql, db::Params::Positional(event_values))
            .await?;
        ...
        for row in event_rows.iter() {
            ...
            let event = GoalRunEvent {
                id: row.get(0)?,
                timestamp: row.get::<i64>(2)? as u64,
                phase: row.get(3)?,
                message: row.get(4)?,
                details: row.get(5)?,
                step_index: row.get::<Option<i64>>(6)?.map(|value| value as usize),
        ...
        if let Some(goal_run_id) = goal_run_id.as_deref() {
            ...
            goal_values.push(db::Value::Text(goal_run_id.to_string()));
        }
        goal_sql.push_str(" ORDER BY updated_at DESC");

        let goal_rows = self
            .read_db
            .query(&goal_sql, db::Params::Positional(goal_values))
            .await?;

        let mut goal_runs = Vec::new();
        for row in goal_rows.iter() {
            let mut goal_run = map_goal_run_row_db(row)?;
        ...
        let goal_run_id = self
            ...
            .query_opt(
                "SELECT id
                 ...
                 ORDER BY updated_at DESC, id DESC
                 LIMIT 1",
                db::db_params![thread_id_owned.as_str()],
            )
            .await?
            .map(|row| row.get::<String>(0))
            .transpose()?;
    ...
    pub(crate) async fn latest_goal_run_repo_context_for_thread(
        ...
        let row = self
            ...
            .await?;
        row.map(|row| -> anyhow::Result<GoalRunRepoContextRef> {
            Ok(GoalRunRepoContextRef {
                id: row.get(0)?,
                session_id: row.get(1)?,
                current_step_index: row.get::<i64>(2)?.max(0) as usize,
            })
        })
        .transpose()
    }
    ...
    pub(crate) async fn list_goal_runs_page(
        ...
        let step_rows = self
            .interactive_read_db
            .query(&step_sql, db::Params::Positional(values.clone()))
            .await?;
        let mut step_map = std::collections::HashMap::<String, Vec<GoalRunStep>>::new();
        for row in step_rows.iter() {
            let goal_run_id = row.get::<String>(1)?;
        ...
        let event_rows = self
            .interactive_read_db
            .query(&event_sql, db::Params::Positional(values.clone()))
            .await?;
        let mut event_map = std::collections::HashMap::<String, Vec<GoalRunEvent>>::new();
        for row in event_rows.iter() {
            let goal_run_id = row.get::<String>(1)?;
        ...
        let goal_rows = self
            .interactive_read_db
            .query(&goal_sql, db::Params::Positional(values))
            .await?;
        ...
        for row in goal_rows.iter() {
            let mut goal_run = GoalRun {
                ...
                created_at: row.get::<i64>(6)? as u64,
                updated_at: row.get::<i64>(7)? as u64,
                started_at: row.get::<Option<i64>>(8)?.map(|v| v as u64),
                completed_at: row.get::<Option<i64>>(9)?.map(|v| v as u64),
                ...
                current_step_owner_profile: None,
                step_owner_overrides: std::collections::BTreeMap::new(),
                replan_count: row.get::<i64>(15)?.max(0) as u32,
                max_replans: row.get::<i64>(16)?.max(0) as u32,
                ...
                child_task_ids: Vec::new(),
                child_task_count: row.get::<i64>(23)?.max(0) as u32,
                approval_count: row.get::<i64>(24)?.max(0) as u32,
                awaiting_approval_id: row.get(25)?,
                policy_fingerprint: row.get(26)?,
                approval_expires_at: row.get::<Option<i64>>(27)?.map(|v| v as u64),
                ...
                events: Vec::new(),
                dossier: None,
                total_prompt_tokens: row.get::<i64>(33)?.max(0) as u64,
                total_completion_tokens: row.get::<i64>(34)?.max(0) as u64,
                estimated_cost_usd: row.get(35)?,
                model_usage: Vec::new(),
                autonomy_level: parse_autonomy_level(
                    &row.get::<Option<String>>(36)?.unwrap_or_default(),
                ),
                authorship_tag: row
                    .get::<Option<String>>(37)?
                    .map(|value| parse_authorship_tag(&value)),
            };
            goal_run.steps = step_map.remove(&goal_run.id).unwrap_or_default();
            goal_run.events = event_map.remove(&goal_run.id).unwrap_or_default();

...
async fn gateway_route_modes_round_trip() -> Result<()> {
    ...
    store
        .upsert_gateway_route_mode("Slack:C123", zorai_protocol::AGENT_ID_SWAROG, 1000)
        .await?;
    store
        .upsert_gateway_route_mode("Discord:999", zorai_protocol::AGENT_ID_RAROG, 1100)
        .await?;
    store
        .upsert_gateway_route_mode("Slack:C123", zorai_protocol::AGENT_ID_RAROG, 1200)
        .await?;
    ...
    assert_eq!(
        ...
        Some(zorai_protocol::AGENT_ID_RAROG)
    );
    assert_eq!(

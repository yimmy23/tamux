...
use helpers::{
    ...
    mark_whatsapp_replay_active_if_cursors, zorai_device_props,
};
#[cfg(test)]
pub(crate) use helpers::{
    parse_whatsapp_cursor, should_enqueue_from_me_whatsapp_message, zorai_self_chat_prefix,
};
...
pub(crate) async fn start_whatsapp_link_native(agent: Arc<AgentEngine>) -> Result<()> {
    ...
    let (device_os, device_version, device_platform) = zorai_device_props();

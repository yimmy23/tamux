...
fn export_skill_writes_agentskills_format() {
    ...
    assert!(!exported.contains("zorai:"));
}

#[test]
fn export_skill_writes_zorai_format() {
    let temp = tempfile::tempdir().expect("tempdir");
    let content = "---\nname: demo\n---\nBody";

    let output = export_skill(content, "zorai", temp.path(), "demo").expect("export succeeds");
    let exported = fs::read_to_string(output).expect("read export");
...
#[test]
fn detect_skill_format_distinguishes_zorai_and_agentskills() {
    assert_eq!(
...
#[test]
fn to_agentskills_format_strips_zorai_extensions() {
    let skill = ZoraiSkillFrontmatter {
    ...
    assert!(!frontmatter.contains("zorai:"));
    assert!(frontmatter.contains("allowed_tools:"));
}

#[test]
fn from_agentskills_format_adds_default_zorai_extensions() {

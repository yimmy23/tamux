...
mod tests {
    ...
    fn memory_worker_builds_global_graph_from_semantic_and_structural_inputs() {
        ...
        assert!(snapshot
            ...
            .nodes
            .iter()
            .any(|node| node.id == "node:package:cargo:zorai-daemon"));
        assert!(snapshot.update_batch.edges.iter().any(|edge| {
            edge.source_node_id == "node:file:Cargo.toml"
                && edge.target_node_id == "node:package:cargo:zorai-daemon"
    ...
    fn memory_worker_materializes_structural_edge_endpoints_before_persisting_edges() {
        let structural_memory = ThreadStructuralMemory {
            ...
            language_hints: vec!["rust".to_string()],
            workspace_seeds: vec![WorkspaceSeed {
                node_id: "node:file:crates/zorai-cli/Cargo.toml".to_string(),
                relative_path: "crates/zorai-cli/Cargo.toml".to_string(),
                kind: "cargo_manifest".to_string(),
            }],
            observed_files: Vec::new(),
            edges: vec![StructuralEdge {
                from: "node:file:crates/zorai-cli/Cargo.toml".to_string(),
                to: "node:file:crates/zorai-cli".to_string(),
        ...
        assert!(snapshot
            ...
            .nodes
            .iter()
            .any(|node| { node.id == "node:file:crates/zorai-cli/Cargo.toml" }));
        assert!(snapshot
            .update_batch
            .nodes
            .iter()
            .any(|node| { node.id == "node:file:crates/zorai-cli" }));
        assert!(snapshot.update_batch.edges.iter().any(|edge| {
            edge.source_node_id == "node:file:crates/zorai-cli/Cargo.toml"
                && edge.target_node_id == "node:file:crates/zorai-cli"

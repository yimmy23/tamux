...
impl AgentEngine {
    ...
    pub async fn stop_gateway(&self) {
        if let Some(sender) = self.gateway_ipc_sender.lock().await.clone() {
            let _ = sender.send(zorai_protocol::DaemonMessage::GatewayShutdownCommand {
                command: zorai_protocol::GatewayShutdownCommand {
    ...
    pub(crate) async fn request_gateway_reload(&self, reason: Option<String>) -> Result<bool> {
        ...
        sender
            .send(zorai_protocol::DaemonMessage::GatewayReloadCommand {
                command: zorai_protocol::GatewayReloadCommand {

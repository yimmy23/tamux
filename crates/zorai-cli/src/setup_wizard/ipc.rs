...
async fn initialize_wizard_connection<T>(
    mut framed: Framed<T, ZoraiCodec>,
) -> Result<Framed<T, ZoraiCodec>>
where
...
pub(super) async fn wizard_connect(
    ...
    initialize_wizard_connection(Framed::new(stream, ZoraiCodec)).await
}
...
pub(crate) async fn ensure_daemon_running() -> Result<()> {
    ...
    let mut cmd = std::process::Command::new(find_sibling_binary("zorai-daemon"));

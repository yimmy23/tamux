...
struct PackageJson {
    ...
    #[serde(default, rename = "zoraiPlugin")]
    zorai_plugin: Option<ZoraiPluginManifest>,
}

#[derive(Debug, Deserialize)]
#[serde(untagged)]
enum ZoraiPluginManifest {
    EntryPath(String),
    Detailed {
        entry: String,
        #[serde(default)]
...
/// Legacy plugin install (npm package with zoraiPlugin field in package.json).
/// Used by `zorai install plugin <package>`.
...
/// Copies from node_modules into ~/.zorai/plugins/{name}/ directory.
...
pub fn install_from_npm(package_spec: &str) -> Result<Vec<(String, String)>> {
    ...
    if nested_plugins.is_empty() {
        ...
        if pkg_json.exists() {
            ...
            if pkg.get("zoraiPlugin").is_some() {
                bail!(
                    "Package '{}' uses legacy zoraiPlugin format. Use 'zorai install plugin {}' for legacy plugins.",
...
/// Install plugin from local directory. Copies files to ~/.zorai/plugins/{name}/.
/// Returns Vec of (dir_name, plugin_name) for nested plugin support.

...
const ZORAI_DATA_DIR_ENV: &str = "ZORAI_DATA_DIR";

pub fn default_tcp_addr() -> String {
    format!("{DEFAULT_TCP_HOST}:{DEFAULT_TCP_PORT}")
}

fn zorai_data_dir_override() -> Option<PathBuf> {
    std::env::var_os(ZORAI_DATA_DIR_ENV)
        .filter(|value| !value.is_empty())
        .map(PathBuf::from)
}

fn legacy_zorai_data_dir() -> PathBuf {
    #[cfg(windows)]
    {
        dirs::data_local_dir()
...
pub fn zorai_data_dir() -> PathBuf {
    if let Some(path) = zorai_data_dir_override() {
        return path;
    }
...
fn migrate_legacy_data_dir() -> std::io::Result<()> {
    if zorai_data_dir_override().is_some() {
        return Ok(());
    }

    let target = zorai_data_dir();
    let legacy = legacy_zorai_data_dir();
...
fn legacy_config_path() -> PathBuf {
    ...
    base.join("zorai").join("config.json")
}

fn zorai_config_path() -> PathBuf {
    let base = dirs::config_dir().unwrap_or_else(|| PathBuf::from("."));
    base.join("zorai").join("config.json")
}

pub fn ensure_zorai_data_dir() -> std::io::Result<PathBuf> {
    migrate_legacy_data_dir()?;
    let dir = zorai_data_dir();
    std::fs::create_dir_all(&dir)?;
    Ok(dir)
}

pub fn log_file_path(file_name: &str) -> PathBuf {
    zorai_data_dir().join(file_name)
}
...
/// User-configurable settings for zorai.
///
...
mod tests {
    ...
    #[test]
    fn zorai_data_dir_honors_env_override() {
        ...
        let temp_dir = std::env::temp_dir().join(format!(
            "zorai-protocol-data-dir-{}-{}",
            std::process::id(),
            test_name
        ));
        let _guard = EnvGuard::set("ZORAI_DATA_DIR", &temp_dir);

        assert_eq!(zorai_data_dir(), temp_dir);
        assert_eq!(
            ensure_zorai_data_dir().expect("override data dir should be created"),
            temp_dir
        );

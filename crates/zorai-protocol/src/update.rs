...
impl ZoraiUpdateStatus {
    ...
    pub fn cli_notice(&self) -> Option<String> {
        self.update_available.then(|| {
            format!(
                "Update available: zorai {} -> {}. Run `zorai upgrade` to install the latest zorai release.",
    ...
    pub fn into_notification(self, timestamp_ms: i64) -> InboxNotification {
        let (title, body, archived_at, deleted_at, read_at) = if self.update_available {
            (
                format!("zorai {} is available", self.latest_version),
                format!(
                    "Installed: {}. Latest: {}. Run `zorai upgrade` to install the latest zorai release.",
                    self.current_version, self.latest_version
                ),
                None,
                None,
            ...
            (
                "zorai is up to date".to_string(),
                format!(
                    "Installed version {} matches the latest available zorai release.",
        ...
        InboxNotification {
            id: ZORAI_UPDATE_NOTIFICATION_ID.to_string(),
            source: "zorai_update".to_string(),
...
mod tests {
    ...
    fn detects_when_latest_version_is_newer() {
        let status = ZoraiUpdateStatus::from_versions("0.2.3", "0.2.4")
    ...
    fn reports_up_to_date_when_versions_match() {
        let status = ZoraiUpdateStatus::from_versions("0.2.4", "0.2.4")
    ...
    fn builds_active_upgrade_notification_when_update_exists() {
        ...
        assert_eq!(notification.id, ZORAI_UPDATE_NOTIFICATION_ID);
        assert_eq!(notification.source, "zorai_update");
        assert_eq!(notification.title, "zorai 0.2.4 is available");
        assert!(
            notification.body.contains("Run `zorai upgrade`"),

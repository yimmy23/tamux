...
mod tests {
    ...
    fn validate_manifest_accepts_max_endpoints_and_settings() {
        ...
        let raw = serde_json::to_vec(&manifest).unwrap();
        let (m, _) = validate_manifest(&raw, &validator).unwrap();
        assert_eq!(m.api.as_ref().unwrap().endpoints.len(), MAX_ENDPOINTS);
        assert_eq!(m.settings.as_ref().unwrap().len(), MAX_SETTINGS);
    }

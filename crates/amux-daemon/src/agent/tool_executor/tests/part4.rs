    ...
    async fn execute_tool_weles_internal_task_does_not_spawn_recursive_governance_review() {
        ...
        assert!(result.is_error, "shell python bypass should still be blocked");
        let review = result
            .weles_review
        ...
        assert!(review
            .reasons
            .iter()
            .any(|reason| reason.contains("daemon-owned WELES internal scope skips recursive governance review")));

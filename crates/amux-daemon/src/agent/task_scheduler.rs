...
pub(super) fn task_lane_key(task: &AgentTask) -> String {
    task.session_id
        .as_deref()
        .filter(|value| !value.trim().is_empty())
...
pub(super) fn current_task_lane_key(task: &AgentTask) -> String {
    task.lane_id.clone().unwrap_or_else(|| task_lane_key(task))
}

pub(super) fn is_task_terminal_status(status: TaskStatus) -> bool {
    matches!(
        status,
...
pub(super) fn task_enforces_workspace_lock(task: &AgentTask) -> bool {
    task.parent_task_id.is_none() && task.parent_thread_id.is_none()
}

pub(super) fn subagent_parent_key(task: &AgentTask) -> Option<String> {
    task.parent_task_id
        .as_deref()
        .filter(|value| !value.trim().is_empty())
...
pub(super) fn project_task_runs(
    tasks: &[AgentTask],
    sessions: &[amux_protocol::SessionInfo],
) -> Vec<AgentRun> {
...
pub(super) fn classify_task(task: &AgentTask) -> &'static str {
    let haystack = format!(
        "{} {} {} {}",
        task.title,
        task.description,
...
pub(super) fn compute_task_backoff_ms(base_delay_ms: u64, retry_count: u32) -> u64 {
    let multiplier = 2u64.saturating_pow(retry_count.saturating_sub(1));
    base_delay_ms.saturating_mul(multiplier).min(5 * 60 * 1000)
}

pub(super) fn describe_scheduled_time(timestamp_ms: u64) -> String {
...
pub(super) fn status_message(task: &AgentTask) -> &'static str {
    match task.status {

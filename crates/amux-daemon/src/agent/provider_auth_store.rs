...
fn provider_auth_db_path() -> Result<PathBuf> {
    ...
    Ok(amux_protocol::ensure_amux_data_dir()?.join("history").join("command-history.db"))
}
...
pub(crate) fn load_provider_auth_state(provider_id: &str, auth_mode: &str) -> Result<Option<Value>> {
    ...
    let row = conn
        ...
        .optional()?;
    row.map(|raw| serde_json::from_str::<Value>(&raw).context("failed to parse provider auth state"))
        .transpose()
}

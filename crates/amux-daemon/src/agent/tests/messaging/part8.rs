...
async fn participant_failed_suggestions_reload() {
    ...
    let suggestions = reloaded.list_thread_participant_suggestions(thread_id).await;
    assert_eq!(suggestions.len(), 1);
    assert_eq!(suggestions[0].status, ThreadParticipantSuggestionStatus::Failed);
    assert_eq!(suggestions[0].error.as_deref(), Some("provider unavailable"));
}
...
async fn participant_observer_failure_keeps_outer_send_successful() {
    ...
    assert!(notice.1.is_some(), "workflow notice should include failure details");

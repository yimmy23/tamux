...
use crate::config::ensure_tamux_data_dir;
...
impl DailyLogWriter {
    pub fn new(file_name: &str) -> io::Result<Self> {
        Self::new_in_directory_with_clock(ensure_tamux_data_dir()?, file_name, || {
            Utc::now().date_naive()
        })
    }
...
mod tests {
    ...
    fn dated_log_file_name_uses_hyphenated_date_before_extension() {
        ...
        let file_name = dated_log_file_name("tamux-daemon.log", date);

        assert_eq!(file_name, "tamux-daemon-2026-04-02.log");
        assert_ne!(file_name, "tamux-daemon.log.2026-04-02");
        assert_ne!(file_name, "tamux-daemon.log2026-04-02");
    }
    ...
    fn daily_log_writer_rolls_over_when_date_changes() {
        ...
        let mut writer =
            DailyLogWriter::new_in_directory_with_clock(tempdir.path(), "tamux-daemon.log", clock)
                .expect("writer");

        writer.write_all(b"first line\n").expect("first write");
        writer.write_all(b"second line\n").expect("second write");

        let day_one = tempdir.path().join("tamux-daemon-2026-04-02.log");
        let day_two = tempdir.path().join("tamux-daemon-2026-04-03.log");
    ...
    fn dated_log_file_path_roots_in_given_directory() {
        let date = NaiveDate::from_ymd_opt(2026, 4, 2).expect("valid date");
        let dir = std::path::Path::new("/tmp/tamux-tests");

        let path = dated_log_file_path(dir, "tamux-gateway.log", date);

        assert_eq!(path, dir.join("tamux-gateway-2026-04-02.log"));
    }
}

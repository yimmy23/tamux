# Zorai Security And Governance Architecture
...
For the broader runtime architecture, see [how-zorai-works.md](./how-zorai-works.md). For the layered memory system that security protects and constrains, see [zorai-memory.md](./zorai-memory.md).

## What Security Means In Zorai

In Zorai, security is the combination of:
...
If a system only looks at a shell string, it is too low-level to govern real autonomy. Zorai is moving the decision point to run/stage/transition semantics.

## Security Starts Before Execution

Zorai security is not only about what happened after the fact.
...
Binary allow/deny is not sufficient for the kind of orchestration Zorai is designed to do.

## Governance Operates On Transitions

One of the most important ideas in Zorai security is that governance applies to transitions, not only to raw commands.
...
Zorai approvals are intended to be bound to real execution conditions rather than treated as permanent blanket permission.
...
Zorai already ships a real adversarial self-critique layer for risky or suspicious tool execution.
...
This is not hypothetical. It is one of the clearest places where Zorai security is already materially ahead of simplistic guardrails.
...
In Zorai, provenance is not merely for later analytics. It is part of the admissibility of risky work.
...
Zorai uses provenance in multiple ways:
...
Zorai already applies secret scrubbing to sensitive critique evidence and operator-facing summaries so that:
...
Zorai security is designed to express more realistic outcomes than “success” or “failure.”
...
Zorai security is not trying to force the operator to click every button.
...
Zorai therefore needs governance that survives:
...
The security and governance architecture in Zorai already includes substantial real surfaces:
...
Zorai security is operator-visible by design.
...
See [zorai-memory.md](./zorai-memory.md) for the memory side of that split.

## The Right Mental Model

The correct mental model for Zorai security is:
...
This is why Zorai security should be described as a governance architecture for autonomous execution, not as a few shell prompts and warnings.

## Related Reading

- [how-zorai-works.md](./how-zorai-works.md)
- [zorai-memory.md](./zorai-memory.md)
- [orchestration-safety-architecture.md](./orchestration-safety-architecture.md)
- [safety-architecture-spec.md](./safety-architecture-spec.md)

...
For implementation detail on the deeper agent internals, see [self-orchestrating-agent.md](./self-orchestrating-agent.md). For onboarding and local setup, see [getting-started.md](./getting-started.md). For the canonical memory architecture, see [zorai-memory.md](./zorai-memory.md). For the canonical security and governance model, see [zorai-security.md](./zorai-security.md).

## System Shape

Zorai is a daemon-first agentic runtime. Terminal sessions are one execution surface, but the core system is the durable agent runtime inside the daemon.
...
- terminal/session lifecycle for shell-backed work
...
- `zorai-mcp` exposes daemon capabilities to external MCP-capable agents.
- `zorai-gateway` lets chat platforms submit work into the daemon and receive results back.

### Session Manager

The session manager is the daemon subsystem responsible for shell-backed execution and terminal-adjacent persistence. It owns:
...
Zorai separates short conversational turns from durable execution.
...
Zorai uses layered memory rather than treating chat history as the only context store.
...
For the full memory architecture, including episodic distillation, forge reflection, dream-state learning, semantic memory, and provenance-backed durable fact handling, see [zorai-memory.md](./zorai-memory.md).

## Persistence Model

Zorai persistence is intentionally mixed: some data is best stored as structured SQLite rows, and some data is better kept as files.
...
See [zorai-memory.md](./zorai-memory.md) for the deeper memory architecture and operator-memory surfaces.
...
Zorai is designed so autonomous execution passes through a real governance plane instead of relying on informal caution.
...
For the full security and governance architecture, see [zorai-security.md](./zorai-security.md).
...
- [zorai-memory.md](./zorai-memory.md): canonical memory architecture
- [zorai-security.md](./zorai-security.md): canonical security and governance architecture

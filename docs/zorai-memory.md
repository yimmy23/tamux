# Zorai Memory Architecture

Zorai memory is not a single file, a vector store, or a convenience summary pasted into the next prompt. It is a layered daemon-owned memory system that combines identity, durable facts, operator profile, workspace task and thread state, procedural memory, recall systems, provenance, and higher-order learning loops.
...
For the broader runtime architecture, see [how-zorai-works.md](./how-zorai-works.md). For governance, approvals, provenance integrity, and execution safety, see [zorai-security.md](./zorai-security.md).

## What Memory Means In Zorai

In Zorai, memory is the full persistence and recall substrate that lets work survive across turns, clients, agents, workspace tasks, and time.
...
If you describe Zorai memory as “three markdown files,” you are describing only the most visible surface, not the actual system.
...
This is one of the core differences between Zorai and disposable terminal chat shells.

## The Memory Stack

Zorai memory is best understood as a stack of layers rather than one store.
...
It is not just a manually edited note file. In Zorai, operator memory is increasingly daemon-owned and structured:
...
This layer is what lets Zorai resume work honestly after restarts and keep continuity across long-running goals.

### 5. Recall Memory

Zorai has multiple retrieval paths rather than one generic “search memory” function.
...
This matters because retrieval in Zorai is contextual. Different questions need different memory surfaces.

### 6. Procedural Memory

Skills are procedural memory.

In Zorai, procedural memory is not secondary. It is one of the strongest forms of reusable knowledge because it stores not just facts, but workable patterns of action.
...
Zorai already has meaningful graph-like and semantic memory surfaces even though the full “Memory Palace” design is not completely shipped.
...
The important point is that Zorai is already past flat chat history, but the full cross-thread graph memory architecture remains an explicit direction for further growth.

### 8. Provenance-Backed Durable Memory

Durable memory writes are not “trust me, the file changed.”

Zorai already tracks memory provenance so the system can answer:
...
This is one of the major places where Zorai memory is substantially stronger than a plain editable context file.
...
Together these form the practical memory system Zorai runs on now.

## The Memory Write Pipeline

Zorai memory writes are curated.
...
Zorai is deliberately trying to preserve durable signal rather than everything that happened.

## Compaction-Aware Memory Preservation

Memory and compaction are tied together.

When older context is about to be compacted, Zorai can preserve durable signal before raw conversational detail falls out of the active window. The current architecture already includes pre-compaction preservation behavior, and the broader memory direction assumes compaction should not mean amnesia.
...
Durable memory in Zorai is not just append-only prose.
...
This is a big part of why Zorai memory can support operator trust instead of behaving like invisible prompt mutation.
...
Zorai already has real implicit-feedback behavior inside the operator-model stack, even if the larger standalone spec is only partially realized.
...
Zorai memory is shared infrastructure, but not every layer is shared in the same way.
...
Zorai memory is tightly tied to trust, but memory is not the same subsystem as governance.
...
See [zorai-security.md](./zorai-security.md) for the governance and security plane.

## The Right Mental Model

The correct mental model for Zorai memory is:
...
This is why Zorai can behave like a long-running operating environment rather than a fresh prompt window on every turn.

## Related Reading

- [how-zorai-works.md](./how-zorai-works.md)
- [zorai-security.md](./zorai-security.md)

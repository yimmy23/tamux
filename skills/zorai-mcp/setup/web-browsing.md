...
description: Configure zorai web browsing with a headless browser (Lightpanda, Chrome, or Chromium)
compatibility:
  - zorai
  - claude-code
allowed_tools:
...
# Setup Web Browsing — configure headless browser for zorai
...
### How zorai web browsing works

The `fetch_url` tool in zorai can use a headless browser to render JavaScript before extracting page text. Without a browser, it falls back to raw HTTP (no JS rendering).
...
### Step 3: Configure zorai

Set the browse provider in zorai settings:
...
Ask the zorai agent to browse a JavaScript-heavy page:
...
zorai auto-detects browsers in this order:

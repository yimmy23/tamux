# Browser Automation — Canvas and sidebar browser control via zorai MCP tools
...
- No CDP (Chrome DevTools Protocol) -- zorai uses Electron webview, not headless Chrome.
...
- The browser uses persistent partition `persist:zorai-browser` -- cookies and session storage carry across panels.

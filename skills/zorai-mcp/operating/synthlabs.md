...
> Route SynthLabs Reasoning Generator work through the right built-in zorai skill. This is a workflow guide for operating an external app with existing zorai tools, not a dedicated SynthLabs tool reference.
...
- `synthlabs-ui-operator` owns visual-only or UI-led tasks that should run through zorai browser tooling.
- Treat this as a content-level integration: use existing zorai terminals, tasks, goals, and browser capabilities rather than assuming dedicated SynthLabs tools exist.
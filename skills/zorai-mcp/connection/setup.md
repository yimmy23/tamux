# Connecting to zorai via MCP

How to register and connect to the zorai daemon through its MCP server interface.

## Agent Rules

- zorai-mcp is a JSON-RPC 2.0 server over stdio transport — all communication happens via stdin/stdout of the spawned process
...
zorai-mcp is a standalone binary that exposes the zorai daemon as a Model Context Protocol (MCP) server. It connects to the running zorai daemon via Unix socket (Linux/macOS) or TCP (Windows) and translates MCP tool calls into daemon commands.
...
Spawn `zorai-mcp` as a subprocess with stdio communication. Send JSON-RPC messages on stdin, read responses from stdout.
...
| Content-Length framing | HTTP-style `Content-Length` header before each message | Set `ZORAI_MCP_FRAMING=content-length` env var |
...
- **zorai daemon must be running** — the `zorai-daemon` process must be active before starting `zorai-mcp`
- **`zorai-mcp` binary must be in PATH** — or provide the full path in your MCP config
- **Socket / port availability:**
  - Linux/macOS: Unix socket at `$XDG_RUNTIME_DIR/zorai-daemon.sock` or `/tmp/zorai-daemon.sock`
...
- **Daemon must be running first.** If `zorai-mcp` fails to start or returns connection errors, ensure the zorai daemon is running. The MCP server needs the daemon socket — it will not work if the daemon has not created it yet.

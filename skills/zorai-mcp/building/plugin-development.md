# Building zorai Plugins

Develop custom plugins with tools, views, and components to extend zorai agent capabilities and UI.
...
zorai supports two plugin models:
...
Installed via CLI:
```bash
zorai install plugin <npm-package-name>
```
...
Plugin metadata stored at `~/.zorai/plugins/registry.json`.
...
Plugins can contribute UI views using zorai's YAML-based declarative UI (CDUI):
...
Views are persisted under `~/.zorai/views/plugins/`.
...
zorai goal runners can automatically generate reusable SKILL.md documents from successful execution trajectories. These are saved as markdown files that describe a repeatable procedure. This is a form of "plugin by documentation" — the agent learns procedures and can replay them in future goals.
...
- The plugin API may evolve — check for breaking changes when upgrading zorai

...
Optuna is a hyperparameter optimization framework with a define-by-run API that dynamically constructs search spaces. It provides state-of-the-art sampling and pruning algorithms, multi-objective support, and seamless parallelization for scaling studies across dozens of workers. Use this skill for automated hyperparameter tuning, neural architecture search, and any black-box optimization of ML training pipelines.

## When to Use This Skill

This skill should be used when:
- Tuning hyperparameters for any ML/DL model (learning rate, batch size, architecture params)
- Running automated neural architecture search (NAS)
- Performing multi-objective optimization (e.g., accuracy vs. latency tradeoffs)
- Setting up distributed hyperparameter search across multiple GPUs/nodes
- Integrating HP tuning with PyTorch Lightning, HuggingFace Transformers, or scikit-learn pipelines
- Using pruning/early-stopping to accelerate expensive training trials
- Visualizing optimization histories and parameter importance
...
**Key Concepts:**
- **Study**: A complete optimization run with a specific objective
- **Trial**: Single evaluation of the objective function with a hyperparameter set
- **Direction**: `"minimize"` or `"maximize"` (default: minimize)
- Use `study.optimize()` which handles sampling, pruning, and result aggregation

### 2. Search Space Definition (suggest API)
...
**Conditional Search Spaces (define-by-run):**
```python
def objective(trial):
...
### 3. Samplers (Search Strategies)

| Sampler | Best For | Key Feature |
|---------|----------|-------------|
